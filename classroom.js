...
function createAssignment(teacherUserId, { classId, courseId, title, type, dueDate }){
  const assignments = readList(ASSIGNMENTS_KEY);
  const assignment = {
    assignmentId: uid('asg'),
    teacherUserId, classId, courseId: courseId || null,
    title, type, dueDate,
    createdAt: new Date().toISOString()
  };
...
function getAssignmentsForStudent(studentUserId){
  ...
  return readList(ASSIGNMENTS_KEY).filter(a => classIds.includes(a.classId));
}
...
function sendMessage({ fromUserId, toUserId = null, classId = null, subject, body }){
